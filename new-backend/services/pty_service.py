...
the captured output, we know the command has finished.

Includes a global session registry so the LangGraph nodes can
...
class PtySession:
    ...
    async def execute_command(
        ...
        the marker string appears in the buffer.
        ...
        try:
            ...
            while True:
                ...
                if idle >= idle_fallback and current_len > 0:
                    logger.warning(
                        "[execute_command] IDLE FALLBACK after %.1fs idle",
                        idle,
                    )
