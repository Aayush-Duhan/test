...
Centralizes all interactions through ChatSnowflakeCortex so both
...
from core.snowflake_auth import SnowflakeAuthConfig, create_snowpark_session
from graph.state import MigrationContext

logger = logging.getLogger(__name__)


def _get_default_sf_params() -> Dict[str, str]:
    """Read default Snowflake connection params from env or config."""
    return {
        "account": os.getenv("SF_ACCOUNT", ""),
        "user": os.getenv("SF_USER", ""),
        "authenticator": os.getenv("SF_AUTHENTICATOR", "externalbrowser"),
...
def build_chat_snowflake_from_context(state: MigrationContext):
    ...
    defaults = _get_default_sf_params()

    sf_account = (state.sf_account or defaults["account"]).strip()
    sf_user = (state.sf_user or defaults["user"]).strip()
    sf_role = (state.sf_role or defaults["role"]).strip()
    sf_warehouse = (state.sf_warehouse or defaults["warehouse"]).strip()
    sf_database = (state.sf_database or defaults["database"]).strip()
    sf_schema = (state.sf_schema or defaults["schema"]).strip()
    sf_authenticator = (
        state.sf_authenticator or defaults["authenticator"]
...
def split_sql_statements(sql_text: str) -> List[str]:
    """
    Split SQL into statements respecting quoted strings and $$...$$ blocks.
    """
    statements: List[str] = []
    buf: List[str] = []
    in_single = False
