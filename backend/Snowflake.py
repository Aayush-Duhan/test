    ...
    if isinstance(value, list):
        ...
        for item in value:
            if isinstance(item, str):
                ...
                if "content" in item:
                    parts.append(_normalize_text(item.get("content")))
                elif "text" in item:
        ...
        if isinstance(choices, list) and choices:
            ...
            if isinstance(choice, dict):
                if "messages" in choice:
                    return _normalize_text(choice.get("messages"))
                if "message" in choice:
                    return _normalize_text(choice.get("message"))
                if "content" in choice:
                    return _normalize_text(choice.get("content"))
                if "text" in choice:
                    return _normalize_text(choice.get("text"))

        if "message" in response:
            return _normalize_text(response.get("message"))
        if "content" in response:
            return _normalize_text(response.get("content"))
...
class ChatSnowflakeCortex(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        # values["session"] = get_from_dict_or_env(
        #     values, "session", "session"
        # )
        values["session"] = values.get("session")
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ChatSnowflakeCortexError(
                f"Error while making request to Snowflake Cortex: {e,sql_stmt}"
            )

        response = json.loads(l_rows[0]["LLM_RESPONSE"])
        ai_message_content = _extract_response_text(response)
